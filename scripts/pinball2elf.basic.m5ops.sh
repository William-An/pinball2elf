...
if  [ $# -eq 3 ];  then
    ...
    sysstate=""
    BNAME=`basename $BASE`
    DEST=$BASE.basic.m5ops.elfie
    if test "$( find $basedir -name "$basename*.sysstate" -print -quit)"
    then
      sysstate=`find $basedir -name "$basename*.sysstate"`
      DEST=$sysstate/$BNAME.basic.elfie
    fi
    cp $INST/basic_m5ops_callbacks.c $TMPDIR/basic_m5ops_callbacks.$$.c
    if [ ! -z $sysstate ];
    then
      ...
      PREOPEN $sysstate $TMPDIR/basic_m5ops_callbacks.$$.c
    else
      echo "WARNING: No sysstate directory exists for $BNAME"
      echo "void preopen_files(){}" >>  $TMPDIR/basic_m5ops_callbacks.$$.c
    fi
    if [ ! -z $sysstate ];
    then
      ...
      then
        ...
        SETHEAP $BRKfile $TMPDIR/basic_m5ops_callbacks.$$.c
      else
    ...
    rm  $TMPDIR/basic_m5ops_callbacks.$$.*
