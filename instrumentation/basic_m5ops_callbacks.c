...
void elfie_on_start(uint64_t num_threads, void* context)
{
   ...
   lte_write(1, s_pid, lte_strlen(s_pid)-1); lte_diprintfe(1, pid, '\n');
   lte_fsync(1);
  lte_syscall(__NR_write, 2, (uint64_t)strp, lte_strlen(strp)-1, 0, 0, 0); 
  lte_fsync(2);
...
lte_td_t elfie_on_thread_start(uint64_t tnum, void* context, uint64_t icount)
{
  ...
  lte_write(1, s_tid, lte_strlen(s_tid)-1); lte_diprintfe(1, tid, '\n');
  lte_syscall(__NR_write, 2, (uint64_t)strt, lte_strlen(strt)-1, 0, 0, 0); 
  lte_fsync(2);
